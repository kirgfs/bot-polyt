...
> Сбор: **2026-09-25**. Документ живой. Новое допущение об API сначала записывается сюда со ссылкой на источник и только потом попадает в код.
...
- **Решение проекта:**
  - бот проверяет geoblock при старте и раз в N минут;
  - при `blocked` или close-only — запрет новых ордеров, `cancel_all`, алерт;
  - никаких обходов через VPN или прокси: это нарушение ToS и риск заморозки средств;
  - оператор и VPS должны находиться в юрисдикции, где торговля разрешена.
