...
- **Эндпоинты CLOB:**
  ...
  - **[ДОПУЩЕНИЕ, мини-бот]** единица `rewardsMaxSpread` / `rewards_max_spread` — **центы**: в примерах формулы на странице наград v задаётся в центах («3¢») [DOC?]. В коде единица — параметр `quoting.rewards_spread_unit` (`config/minibot.yaml`); в бумажном отчёте печатаем и v, и наш фактический полуспред, чтобы проверить по интерфейсу Polymarket (чек-лист, п. 20);
...
- **Оценка наград в бумажном режиме (мини-бот).** Раз в минуту считаем `Q_min` наших бумажных ордеров по формуле выше (b = 1, до матча) и `Q_min` всей видимой книги L2 так, будто это один мейкер. Сумма `Q_min` по отдельным мейкерам не больше `Q_min` агрегированной книги, поэтому наша доля `Q_наш / (Q_наш + Q_книги)` — **оценка снизу**. Награда за минуту = доля × дневная ставка / 1440. Ставка — сумма `clobRewards[].rewardsDailyRate` из Gamma. Это оценка, а не начисление: реальное начисление видно только через `/rewards/user*` на живом счёте.
...
- **Лиги в футболе (мини-бот, решение 9) — [НЕ ПРОВЕРЕНО].** Предполагаем, что у матчевых событий лиги есть тег лиги, как у тенниса есть тег `tennis`. Slug-и в `config/minibot.yaml` (`serie-a`, `la-liga`, `ligue-1`, `eredivisie`) — **догадка**: из облака Gamma недоступна. Защита: мини-бот не стартует, если slug не находится через `/tags/slug/{slug}`; `polybot minibot --dry-run` печатает частоту тегов на футбольных матчах, чтобы выбрать правильные slug-и (чек-лист, п. 19).
- **Расчёт позиции после матча (бумажный режим) — [ДОПУЩЕНИЕ].** `GET /markets/{id}` (эндпоинт из [SDK], см. выше): когда `closed == true`, `outcomePrices` считаем итоговой выплатой за акцию каждого исхода (`["1","0"]`, при 50-50 — `["0.5","0.5"]`). Пока рынок не закрыт или цены в сумме не дают 1 (значит, это ещё не выплата), позиция остаётся «нерассчитанной» и оценивается по последнему микропрайсу; мини-бот спрашивает рынок раз в 10 минут, начиная через 2 ч после `gameStartTime` (чек-лист, п. 21).
...
- **Футбол:** три бинарных рынка — победа A, ничья, победа B; обычно это neg-risk событие. Итог считается по основному времени с добавленным. [2nd] → сверить по `description`.
  - Мини-бот котирует только токен **Yes** каждого из трёх рынков; исход ищется по тексту `"Yes"` в `outcomes` (регистр не важен). Рынок без исхода `Yes` пропускается [ДОПУЩЕНИЕ о формате, проверяется в рантайме].
  - Состав команд объявляют примерно за час до начала — это информационное событие. Поэтому котировки снимаются за `pull_before_start_min` (по умолчанию 75 мин) до `gameStartTime`. Это наше правило риска, не факт об API.
...
| 19 | Slug-и тегов лиг: Серия А, Ла Лига, Лига 1, Эредивизи | — | `polybot minibot --dry-run` на VPS: частота тегов на футбольных матчах; при ошибке мини-бот не стартует |
| 20 | Единица `rewardsMaxSpread` (центы?) | DOC? | Сравнить v из Gamma с интерфейсом рынка на polymarket.com; бумажный отчёт печатает v и наш полуспред |
| 21 | Итоговые `outcomePrices` закрытого рынка = выплата | — | Бумажный отчёт: расчёт по закрытым рынкам сверить с результатами матчей |
| 22 | Исходы футбольного moneyline — `Yes`/`No`, значение `sportsMarketType` | 2nd | `polybot minibot --dry-run`: типы рынков и исходы |
