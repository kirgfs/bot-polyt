...
def render(run: ScoutRun, top: int = 10) -> str:
    ...
    if not run.copybot.verified:
        ...
        lines += [
            f"> ⚠ **{bot}:** семантика полей взята из документации бота (`copybot_fields.yaml`), но не описаны: "
            f"{missing}. Бэктест считает их выключенными. Сообщение «НАСТРОЙКИ» не выдаётся, пока они не подтверждены.",
            "",
        ]
    lines += ["## Коротко", ""]
    lines.append(
    ...
    lines += [
        f"- Copy-бот — {run.copybot.bot.name or 'не указан'}, семантика его полей — `copybot_fields.yaml` по его "
        f"документации: покупка по формуле балансов, продажа той же долей, минимум копии "
        f"{fmt_usd(run.copybot.semantics.min_copy_usd)} (мельче — ожидание {run.copybot.semantics.small_wait_s:g} с, "
        f"потом Skip/Buy), комиссия бота {run.copybot.bot.fee_bps:g} б.п., Max Leverage.",
        ...
        "- Пул: «поиск» среди прибыльных в полосе копируемых (капитал и месячный оборот в разумных пределах) плюс "
        "случайная контрольная выборка из той же полосы. Проверка всего процесса — только на контрольной. Остаточный "
        "survivorship bias: правила попадания в неофициальный лидерборд неизвестны (см. docs/methodology.md).",
