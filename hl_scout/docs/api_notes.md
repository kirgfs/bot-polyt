...
| `userRole` | `user` | `role`: `missing` / `user` / `agent` / `vault` / `subAccount`. У `agent` есть `data.user` — адрес мастер-аккаунта, у `subAccount` — `data.master`. Частая ошибка — запрашивать данные по адресу API-кошелька (агента): ответ будет пустым, нужен адрес мастер-аккаунта | [SDK метод][SDK-TS][DOC-S] |
| `allMids` | `dex` (`""`) | `{монета: "mid"}` для всех торгуемых монет; weight 2 — самый дешёвый способ получить текущие цены | [SDK][DOC-S] |
...
- Подтверждение подписки: кадр `{"channel": "subscriptionResponse", "data": {"method": "subscribe", "subscription": <эхо>}}`. Ошибки приходят кадром `{"channel": "error", "data": "<текст>"}`. После переподключения сервер может отклонить повторную подписку, поэтому подтверждения надо проверять заново [SDK-TS `transport/websocket`].
- Ping: `{"method": "ping"}`, ответ — канал `pong`.
  - **Сервер закрывает соединение, молчащее ~60 с** [SDK-TS `_keepAlive`].
...
- `{"type": "userFills", "user": ...}` → канал `userFills`, `data = {user, isSnapshot, fills[]}` [SDK `types.py`].
  - Первый кадр после (пере)подписки — снимок с `isSnapshot: true` [SDK-TS]. Это не новые сделки, а история.
...
- API-кошелёк (агент) подписывает действия от имени мастер-аккаунта или его субаккаунтов. Он может торговать, но **не может выводить средства** [DOC-S: «Nonces and API wallets»][2nd].
- Безымянный агент отключается, когда одобрен новый безымянный агент. Именованный — когда одобрен агент с тем же именем [DOC-S]. Вывод для пользователя: если два бота подключены безымянными агентами, второй молча отключит первый. Каждому боту нужно своё имя агента.
...
- [ ] WS: приходит ли `subscriptionResponse` на каждую подписку (§5); закрывает ли сервер молчащее соединение через ~60 с.
